...
export default function CategoryList({ title, categories, icon }) {
  const categoryList = categories.map((category, index) => (
    <li key={index} className="mx-1 mb-2 capitalize">
      <Link href={`/category/${category.toLowerCase()}`}>
        <a className="flex flex-row rounded-full bg-sky-600 py-2 px-3 text-sm text-white shadow-md shadow-black/10 lg:hover:text-yellow-100">
  ...
  if (categories.length != 0)
    if (title === undefined) {
      ...
      return (
        <>
          <h2 className="flex items-center space-x-2 py-2 px-4 font-semibold text-sky-100/70 md:px-12 md:text-lg">
